...
class EconomicDataLoader:
    ...
    def load_economic_datasets(self) -> Dict[str, pd.DataFrame]:
        ...
        # Load HRST dataset
        hrst_path = self.data_dir / getattr(
            self.config,
            "hrst_file_path",
            "L2_estat_hrst_st_rcat_filtered_en/estat_hrst_st_rcat_filtered_en.csv",
        )
        if hrst_path.exists():
    ...
    def _process_gdp_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        nl_data_mio = nl_data[nl_data["unit"].str.contains("MIO_EUR")].reset_index(
            drop=True
        )
        ...
        processed = latest_data[
            ["geo", "OBS_VALUE", "unit", "Geopolitical entity (reporting)"]
        ].copy()
        ...
        logger.info(
            f"Processed GDP data: {len(processed)} regions for year {latest_year}"
        )
        return processed
    ...
    def _process_freight_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        aggregated = total_data[
            ["geo", "OBS_VALUE", "unit", "Geopolitical entity (reporting)"]
        ].copy()
        aggregated.columns = ["nuts_code", "freight_value", "unit", "region"]
        aggregated["freight_value"] = pd.to_numeric(
            aggregated["freight_value"], errors="coerce"
        )
        ...
        if not aggregated.empty and aggregated["unit"].iloc[0] == "THS_T":
            ...
            aggregated["unit"] = "T"
            logger.info(
                "Converted NUTS freight data from thousands of tonnes to tonnes"
            )

        logger.info(
            f"Processed freight data: {len(aggregated)} regions for year {latest_year}"
        )
        logger.info(
            f"Total NUTS freight volume: {aggregated['freight_value'].sum():,.0f} tonnes"
        )
        return aggregated
    ...
    def _process_hrst_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        processed["hrst_value"] = pd.to_numeric(
            processed["hrst_value"], errors="coerce"
        )
        ...
        logger.info(
            f"Processed HRST data: {len(processed)} regions for year {latest_year}"
        )
        return processed
    ...
    def _load_maritime_freight_data(self) -> pd.DataFrame:
        ...
        try:
            nuts_freight_data, enhanced_datasets = (
                self.freight_processor.load_and_process_freight_data()
            )
...
class NUTSDataMapper:
    ...
    def join_economic_data(
        self,
        nuts_gdfs: Dict[str, gpd.GeoDataFrame],
        ...
        loading_nuts_mapping_path = os.path.join(
            self.config.data_dir, "Mapping_NL_NUTS_2021_2024.xlsx"
        )
        mapping_df = (
            pd.read_excel(loading_nuts_mapping_path)
            if os.path.exists(loading_nuts_mapping_path)
            else None
        )
        ...
        for dataset_name, df in economic_data.items():
            ...
            if nuts_level not in nuts_gdfs:
                logger.warning(
                    f"NUTS level {nuts_level} not available for dataset {dataset_name}"
                )
                continue
            ...
            if nuts_code_col is None:
                raise ValueError(
                    f"Could not find NUTS code column in {nuts_level} shapefile"
                )
            ...
            if value_col not in economic_df.columns:
                logger.warning(
                    f"Value column '{value_col}' not found in {dataset_name} data"
                )
                continue
            ...
            if mapping_df is not None and not mapping_df.empty:
                ...
                if (
                    old_col
                    and new_col
                    and old_col in mapping_df.columns
                    and new_col in mapping_df.columns
                ):
                    mapping_dict = dict(zip(mapping_df[old_col], mapping_df[new_col]))
                    economic_df["nuts_code"] = (
                        economic_df["nuts_code"]
                        .map(mapping_dict)
                        .fillna(economic_df["nuts_code"])
                    )
                    logger.info(
                        f"Applied NUTS code mapping: {len(mapping_dict)} mappings"
                    )

            # Perform the join
            joined_gdf = nuts_gdf.merge(
                economic_df, left_on=nuts_code_col, right_on="nuts_code", how="left"
            )
            ...
            if missing_regions > 0:
                logger.warning(
                    f"Some NUTS regions missing {dataset_name} data - check NUTS code matching"
                )

                # Log missing regions for debugging
                missing_nuts_codes = joined_gdf[joined_gdf[value_col].isna()][
                    nuts_code_col
                ].tolist()
                logger.debug(
                    f"Missing NUTS codes for {dataset_name}: {missing_nuts_codes}"
                )
            ...
            if joined_regions > 0:
                ...
                logger.info(
                    f"Successfully joined {dataset_name} data with {joined_regions} regions"
                )
            else:
                logger.error(
                    f"No regions joined for {dataset_name} - check data compatibility"
                )
...
class EconomicDistributor:
    ...
    def rasterize_nuts_regions(
        self, nuts_gdf: gpd.GeoDataFrame, exposition_meta: dict, economic_variable: str
        ...
        shapes = [
            (geom, value)
            for geom, value in zip(nuts_gdf.geometry, nuts_gdf[value_column])
        ...
        logger.info(
            f"Rasterized {economic_variable}: shape {raster.shape}, "
            f"min={np.nanmin(raster)}, max={np.nanmax(raster)}"
        )

        return raster, meta

    def distribute_with_exposition(
        self,
        economic_raster: np.ndarray,
        ...
        """
        logger.info(
            "Distributing economic values using exposition layer with enhanced freight integration"
        )
        ...
        if enhanced_freight_datasets and "port_freight" in enhanced_freight_datasets:
            logger.info(
                "Applying enhanced port freight data using centralized approaches"
            )
        ...
        logger.info(
            f"Final distributed economic values: min={np.nanmin(distributed)}, "
            f"max={np.nanmax(distributed)}, mean={np.nanmean(distributed)}"
        )

        return distributed

    def _apply_nuts_distribution(
        self, economic_raster: np.ndarray, exposition_layer: np.ndarray
    ) -> np.ndarray:
    ...
    def _apply_port_freight_enhancement(
        self, distributed_base: np.ndarray, port_freight_data: pd.DataFrame
    ) -> np.ndarray:
        ...
        port_raster = self._rasterize_port_freight(
            port_freight_data, distributed_base.shape
        )
        ...
        if np.any(port_mask):
            ...
            logger.info(
                f"Added precise port freight data to existing NUTS distribution at {np.sum(port_mask)} port pixels"
            )
            logger.info(
                f"Total port freight added: {port_raster[port_mask].sum():,.0f}"
            )

        return enhanced_distributed

    def _rasterize_port_freight(
        self, port_freight_data: pd.DataFrame, target_shape: Tuple[int, int]
    ) -> np.ndarray:
        ...
        try:
            ...
            logger.info(
                f"Using port rasterization resolution: {port_resolution}m (half of {target_resolution}m target)"
            )
            ...
            logger.info(
                f"High-resolution raster shape: {hr_shape} (vs target: {target_shape})"
            )
            ...
            for _, row in port_freight_data.iterrows():
                if "geometry" in row and "freight_value" in row:
                    ...
                    logger.debug(
                        f"Processing port {port_id}: freight={freight_value}, geometry_valid={row['geometry'] is not None}"
                    )
            ...
            logger.info(
                f"Rasterized {len(port_freight_data)} ports with area-based freight distribution"
            )
...
class RelevanceLayer:
    ...
    def calculate_relevance(
        self, layers_to_generate: List[str] = None
        ...
        if layers_to_generate:
            target_economic_variables = [
                var for var in target_economic_variables if var in layers_to_generate
            ]
            logger.info(
                f"Generating only requested indicators: {target_economic_variables}"
            )

        # Filter to available indicators
        available_variables = [
            var for var in target_economic_variables if var in nuts_economic_gdfs
        ]
        logger.info(f"Processing available indicators: {available_variables}")

        if not available_variables:
            raise ValueError(
                "No economic variables available for relevance calculation"
            )
        ...
        for var_name in available_variables:
            ...
            economic_raster, raster_meta = (
                self.economic_distributor.rasterize_nuts_regions(
            ...
            if economic_exposition_data.shape != economic_raster.shape:
                ...
                economic_exposition_data = self.transformer.ensure_alignment(
                    economic_exposition_data,
                    exposition_meta["transform"],
                    raster_meta["transform"],
                    economic_raster.shape,
            ...
            if var_name == "freight":
                if hasattr(self.economic_data_loader, "enhanced_freight_datasets"):
                    enhanced_datasets = (
                        self.economic_data_loader.enhanced_freight_datasets
                    )
                    logger.info("Using enhanced freight datasets for distribution")

            # Apply economic distribution with exposition weighting
            distributed_economic_raster = (
                self.economic_distributor.distribute_with_exposition(
                    economic_raster,
                    economic_exposition_data,
                    enhanced_datasets,
                    raster_meta,
            ...
            if np.any(valid_mask):
                ...
                logger.info(
                    f"Processed {var_name}: min={min_val:.6f}, max={max_val:.6f}, mean={mean_val:.6f}"
                )
        ...
        if len(relevance_layers) > 1:
            ...
            for var_name, layer_data in relevance_layers.items():
                ...
                if np.any(valid_mask):
                    normalized_layers[var_name] = (
                        self.normalizer.normalize_economic_data(layer_data, valid_mask)
                    )
                else:
            ...
            weights = np.ones(len(normalized_layers)) / len(
                normalized_layers
            )  # Equal weights
        ...
        logger.info(
            f"Completed relevance calculation for {len(relevance_layers)} layers"
        )
    ...
    def _get_exposition_metadata(self) -> dict:
        ...
        if default_path.exists():
            ...
            logger.info(
                "No existing exposition layer found, generating metadata from config"
            )
    ...
    def _get_economic_exposition_layer(self, dataset_name: str) -> np.ndarray:
        ...
        tif_path = (
            ...
            / "exposition"
            / "tif"
            / f"exposition_{dataset_name}.tif"
        )

        if tif_path.exists():
            logger.info(
                f"Loading existing economic exposition layer for {dataset_name}"
            )
            ...
            if dataset_name not in economic_weights:
                logger.warning(
                    f"No specific exposition weights found for {dataset_name}, using default"
                )
            ...
            exposition_data, meta = (
                self.exposition_layer.create_economic_exposition_layer(
            ...
            self.exposition_layer.save_exposition_layer(
                exposition_data, meta, str(tif_path)
            )
            logger.info(
                f"Created and saved economic exposition layer for {dataset_name}"
            )

            return exposition_data

    def run_freight_relevance_only(
        self, visualize: bool = True, export_tif: bool = True
    ) -> Dict[str, np.ndarray]:
        """
        ...
        """
        logger.info(
            "Generating freight relevance layer only with enhanced Zeevart data"
        )
    ...
    def save_relevance_layers(
        self, relevance_layers: Dict[str, np.ndarray], meta: dict
    ):
        ...
        output_meta.update(
            {"driver": "GTiff", "dtype": "float32", "count": 1, "nodata": None}
        )
    ...
    def visualize_relevance_layers(
        self,
        ...
        try:
            with rasterio.open(self.config.land_mass_path) as src:
                if meta and "transform" in meta:
                    ...
                    logger.info(
                        "Loaded and transformed land mask for relevance visualizations"
                    )
                else:
                    logger.warning("No metadata available for land mask transformation")
        except Exception as e:
            logger.warning(
                f"Could not load land mask for relevance visualizations: {e}"
            )
    ...
    def run_relevance_analysis(
        ...
        layers_to_generate: List[str] = None,
    ) -> Dict[str, np.ndarray]:
