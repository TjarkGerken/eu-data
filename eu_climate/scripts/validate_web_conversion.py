...
class WebConversionValidator:
    ...
    def find_file_pairs(
        ...
        for gpkg_file in self.output_dir.glob("**/gpkg/*.gpkg"):
            ...
            mbtiles_file = (
                gpkg_file.parent.parent / "web" / "mvt" / f"{gpkg_file.stem}.mbtiles"
            )
        ...
        self.logger.info(
            f"Found {len(raster_pairs)} raster pairs and {len(vector_pairs)} vector pairs"
        )
    ...
    def validate_raster_conversion(self, original_path: Path, cog_path: Path) -> Dict:
        ...
        result = {"test": test_name, "passed": True, "errors": [], "details": {}}
            ...
            with rasterio.open(original_path) as orig, rasterio.open(cog_path) as cog:
                ...
                if orig.shape != cog.shape:
                    result["errors"].append(
                        f"Shape mismatch: {orig.shape} → {cog.shape}"
                    )
                    result["passed"] = False
                else:
                ...
                if not all(
                    abs(a - b) < bounds_tolerance
                    for a, b in zip(orig_bounds, cog_bounds)
                ):
                    result["errors"].append(
                        f"Bounds mismatch: {orig_bounds} → {cog_bounds}"
                    )
                    result["passed"] = False
                else:
                ...
                if orig.dtypes != cog.dtypes:
                    result["errors"].append(
                        f"Data type mismatch: {orig.dtypes} → {cog.dtypes}"
                    )
                    result["passed"] = False
                else:
                ...
                try:
                    ...
                    if not np.array_equal(orig_sample, cog_sample, equal_nan=True):
                        ...
                        if max_diff > 1e-6:
                            result["errors"].append(
                                f"Pixel value mismatch, max diff: {max_diff}"
                            )
                            result["passed"] = False
                        else:
                ...
                if cog_profile.get("tiled", False):
                    ...
                    result["details"]["block_size"] = (
                        f"{cog_profile.get('blockxsize', 'unknown')}x{cog_profile.get('blockysize', 'unknown')}"
                    )
                else:
    ...
    def validate_vector_conversion(
        self, original_path: Path, mbtiles_path: Path
    ) -> Dict:
        ...
        try:
            ...
            result["details"]["geometry_types"] = list(
                gdf_orig.geometry.geom_type.unique()
            )
                ...
                try:
                    ...
                    bounds_match = all(
                        ...
                        for expected, actual in zip(
                            expected_wgs84_bounds, mbtiles_bounds
                        )
                    )
                    ...
                    if bounds_match:
                        ...
                        max_diff = max(
                            abs(e - a)
                            for e, a in zip(expected_wgs84_bounds, mbtiles_bounds)
                        )
                        result["errors"].append(
                            f"Bounds mismatch, max diff: {max_diff:.6f}°"
                        )
            ...
            if "minzoom" in metadata and "maxzoom" in metadata:
                ...
                if min_zoom < 0 or max_zoom > 18 or min_zoom > max_zoom:
                    result["errors"].append(
                        f"Invalid zoom range: {min_zoom}-{max_zoom}"
                    )
                    result["passed"] = False
            ...
            result["details"]["file_sizes_mb"] = {
                ...
                "size_ratio": round(
                    mbtiles_size_mb / orig_size_mb if orig_size_mb > 0 else 1, 2
                ),
            }
            ...
            if len(invalid_geoms) > 0:
                ...
                result["errors"].append(
                    f"Found {len(invalid_geoms)} invalid geometries in original data"
                )
    ...
    def run_validation(self) -> Dict:
        ...
        if not raster_pairs and not vector_pairs:
            self.logger.warning(
                "No converted file pairs found. Run --web-conversion first."
            )
        ...
        total_passed = (
            self.results["raster"]["passed"] + self.results["vector"]["passed"]
        )
        total_failed = (
            self.results["raster"]["failed"] + self.results["vector"]["failed"]
        )
        ...
        self.results["summary"] = {
            ...
            "raster_success_rate": round(
                self.results["raster"]["passed"]
                / self.results["raster"]["total"]
                * 100,
                1,
            )
            if self.results["raster"]["total"] > 0
            else 0,
            "vector_success_rate": round(
                self.results["vector"]["passed"]
    ...
    def print_summary(self):
        ...
        if self.results["raster"]["total"] > 0:
            ...
            passed_raster = [
                r for r in self.results["raster"]["details"] if r["passed"]
            ]
            if passed_raster:
        ...
        if self.results["vector"]["total"] > 0:
            ...
            # Show common details
            passed_vector = [
                r for r in self.results["vector"]["details"] if r["passed"]
            ]
            if passed_vector:
                example = passed_vector[0]["details"]
                if "zoom_range" in example:
                    print(
                        f"   Zoom Range: {example['zoom_range'][0]}-{example['zoom_range'][1]}"
                    )
        ...
        for category in ["raster", "vector"]:
            failed_tests.extend(
                [r for r in self.results[category]["details"] if not r["passed"]]
            )
        ...
        if summary["success_rate"] >= 95:
            ...
            print(
                "⚠️  FAIR: Web conversion has some accuracy issues that should be addressed."
            )
        else:
...
def main():
    ...
    parser.add_argument(
        "--save-report", type=Path, help="Save detailed JSON report to file"
    )

    args = parser.parse_args()

    # Initialize validator
    validator = WebConversionValidator(output_dir=args.output_dir, verbose=args.verbose)
