...
def regenerate_mbtiles_optimized(config):
    ...
    for gpkg_file in cluster_files:
        ...
        try:
            ...
            simplification = mvt_settings.get(
                "simplification", "drop-densest-as-needed"
            )

            logger.info(
                f"Using optimized settings: zoom {min_zoom}-{max_zoom}, {simplification}"
            )
            ...
            if success and output_path.exists():
                ...
                if file_size_mb > 50:
                    logger.warning(
                        f"⚠️  Large file ({file_size_mb:.2f} MB) - consider further optimization"
                    )
                else:
    ...
    logger.info(
        f"Average size: {total_size_mb / max(success_count, 1):.2f} MB per file"
    )
