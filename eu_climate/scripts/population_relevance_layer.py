...
class PopulationRelevanceLayer:
    ...
    def load_population_data(self):
        ...
        logger.info(f"  Area scaling applied: {metadata.get('area_scaling_applied', False)}")
        ...
        logger.info(f"  Population density range: {valid_data.min():.2f} - {valid_data.max():.2f} persons/pixel")
    ...
    def normalize_population_data(self, population_data: np.ndarray) -> np.ndarray:
        ...
        normalized_data = self.normalizer.normalize_economic_data(population_data, valid_mask)
    ...
    def save_relevance_layer(self, relevance_data: np.ndarray, metadata: dict, layer_name: str = "population"):
    ...
    def visualize_relevance_layer(self, relevance_data: np.ndarray, metadata: dict, layer_name: str = "population"):
        ...
        try:
            with rasterio.open(self.config.land_mass_path) as src:
                land_mask, _ = rasterio.warp.reproject(
                    source=src.read(1),
                    destination=np.zeros((metadata['height'], metadata['width']), dtype=np.uint8),
...
def main():
    ...
    try:
        ...
        logger.info(f"✓ Used 2025 GHS population data with corrected resolution handling")
