...
class VierkantStatsProcessor:
    ...
    def load_vierkant_vector_data(self) -> gpd.GeoDataFrame:
        ...
        logger.info(
            f"Loaded {len(vierkant_gdf)} grid polygons from Vierkantstatistieken"
        )
        ...
        for column in required_columns:
            if column not in vierkant_gdf.columns:
                raise ValueError(
                    f"Required column '{column}' not found in Vierkantstatistieken data"
                )

        logger.info(f"All required columns found: {required_columns}")
        return vierkant_gdf

    def create_socioeconomic_index_from_vector(
        self, vierkant_gdf: gpd.GeoDataFrame
    ) -> gpd.GeoDataFrame:
        ...
        logger.info(
            f"Using multipliers - Inhabitants: {inhabitants_multiplier}, "
            f"Household size: {household_size_multiplier}, House price: {house_price_multiplier}"
        )

        vierkant_processed = vierkant_gdf.copy()

        inhabitants = self.handle_vector_nodata_values(
            vierkant_processed["aantal_inwoners"]
        )
        household_size = self.handle_vector_nodata_values(
            vierkant_processed["gemiddelde_huishoudensgrootte"]
        )
        house_price = self.handle_vector_nodata_values(
            vierkant_processed["gemiddelde_woz_waarde_woning"]
        )

        if (
            inhabitants.max() == 0
            and household_size.max() == 0
            and house_price.max() == 0
        ):
        ...
        logger.info(
            f"Socioeconomic index statistics - "
    ...
    def convert_vector_to_raster_using_central_transformer(
        self, vierkant_gdf: gpd.GeoDataFrame
    ) -> Tuple[np.ndarray, Dict]:
        """Convert vector socioeconomic index to raster using central transformer."""
        logger.info(
            "Converting vector socioeconomic index to raster using central approach"
        )
        ...
        transformed_gdf = self.ensure_correct_coordinate_system(
            vierkant_gdf, target_crs
        )
        temp_raster_path = self.create_temporary_raster_file()

        try:
            raster_100m = self.create_100m_raster_from_vector(
                transformed_gdf, reference_bounds, target_crs
            )
            self.save_raster_to_temporary_file(
                raster_100m, reference_bounds, target_crs, temp_raster_path
            )
            return self.transform_using_central_transformer(
                temp_raster_path, reference_bounds
            )
        finally:
            self.cleanup_temporary_file(temp_raster_path)

    def ensure_correct_coordinate_system(
        self, vierkant_gdf: gpd.GeoDataFrame, target_crs: rasterio.crs.CRS
    ) -> gpd.GeoDataFrame:
    ...
    def create_100m_raster_from_vector(
        self,
        vierkant_gdf: gpd.GeoDataFrame,
        ...
        return rasterio.features.rasterize(
            [
                ...
                for geom, value in zip(
                    vierkant_gdf.geometry, vierkant_gdf["socioeconomic_index"]
                )
            ],
    ...
    def save_raster_to_temporary_file(
        self,
    ...
    def transform_using_central_transformer(
        self, temp_raster_path: Path, reference_bounds: Tuple
    ) -> Tuple[np.ndarray, Dict]:
        ...
        logger.info(
            f"Final raster - Shape: {final_raster.shape}, "
    ...
    def process_vierkant_stats(self) -> Tuple[np.ndarray, Dict]:
        ...
        final_raster, final_meta = (
            self.convert_vector_to_raster_using_central_transformer(vierkant_with_index)
        )

        logger.info(
            "Vierkantstatistieken vector processing pipeline completed successfully"
        )
        return final_raster, final_meta
