...
exposition_cmap = LinearSegmentedColormap.from_list(
    "exposition_colors", exposition_colors
)
economic_cmap = LinearSegmentedColormap.from_list(
    "economic_risk_colors", economic_risk_colors
)
hazard_cmap = LinearSegmentedColormap.from_list(
    "hazard_risk_colors", hazard_risk_colors
)
...
class LayerVisualizer:
    ...
    def get_port_boundaries(self) -> Optional[gpd.GeoDataFrame]:
        ...
        try:
            ...
            try:
                ...
                logger.info(
                    f"Loaded NUTS-L3 boundaries for port clipping: {len(nuts_gdf)} regions"
                )
                ...
                logger.info(
                    f"Clipped ports for visualization: {len(ports_in_study_area)} ports within Netherlands boundaries (from {len(port_gdf)} total)"
                )

                if len(ports_in_study_area) == 0:
                    logger.warning(
                        "No ports found within the study area boundaries for visualization"
                    )
                    return None
                ...
                logger.warning(
                    f"Could not load NUTS boundaries for port clipping in visualization: {e}"
                )
    ...
    def get_nl_forecast_boundaries(self) -> Optional[gpd.GeoDataFrame]:
        ...
        nl_forecast_path = (
            self.config.data_dir / self.config.config["file_paths"]["nl_forecast_file"]
        )
    ...
    def get_raster_extent(
        self, data: np.ndarray, meta: dict
    ...
    def add_port_overlay(
        self, ax, port_gdf: Optional[gpd.GeoDataFrame], show_buffer: bool = False
    ):
        ...
        if port_gdf is not None:
            ...
            if show_buffer:
                ...
                clean_buffers = gpd.GeoDataFrame(
                    geometry=buffer_geometries, crs=port_gdf.crs
                )

                # Remove areas that overlap with actual port polygons
                port_union = unary_union(port_gdf.geometry.tolist())
                clean_buffers["geometry"] = clean_buffers.geometry.difference(
                    port_union
                )
                ...
                if len(clean_buffers) > 0:
                    ...
                    logger.info(
                        f"Plotted {len(clean_buffers)} non-overlapping buffer zones"
                    )
                else:
            ...
            logger.info(
                f"Plotted {len(port_gdf)} port polygons with precedence over buffers"
            )
    ...
    def create_standard_colorbar(self, im, ax, label: str, shrink: float = 0.8):
        ...
        cbar.set_label(
            label, rotation=270, labelpad=15, fontsize=ScientificStyle.LABEL_SIZE
        )
        cbar.ax.tick_params(labelsize=ScientificStyle.TICK_SIZE)
        return cbar

    def visualize_exposition_layer(
        self,
        data: np.ndarray,
        meta: dict,
        ...
        zones = self.create_zone_classification(
            data.shape, meta["transform"], land_mask
        )
        ...
        ax.imshow(
            ...
            aspect="equal",
            extent=extent,
            vmin=ScientificStyle.WATER_VALUE,
            vmax=ScientificStyle.LAND_BASE_VALUE,
            alpha=1.0,
        )

        # Create data overlay only for Netherlands land areas
        netherlands_mask = (zones == ScientificStyle.LAND_BASE_VALUE) & (
            ~np.isnan(data)
        )
        ...
        legend_patches = [
            ...
            mpatches.Patch(
                color=ScientificStyle.LAND_OUTSIDE_COLOR, label="Outside Netherlands"
            ),
        ]
        ...
        if show_ports:
            legend_patches.extend(
                [
                    mpatches.Patch(
                        color=ScientificStyle.PORT_COLOR, alpha=0.5, label="Port Areas"
                    )
                ]
            )
            if show_port_buffers:
                ...
                legend_patches.append(
                    mpatches.Patch(
                        color=ScientificStyle.PORT_BUFFER_COLOR,
                        alpha=0.3,
                        label=f"Port Buffer Zones ({buffer_distance}m)",
                    )
                )
    ...
    def visualize_hazard_scenario(
        ...
        # Create zone classification for background
        zones = self.create_zone_classification(
            dem_data.shape, meta["transform"], land_mask
        )
        ...
        flood_overlay[netherlands_mask & (flood_mask > 0)] = flood_mask[
            netherlands_mask & (flood_mask > 0)
        ]
        ...
        if river_polygon_network is not None:
            river_polygon_network.plot(
                ax=ax, facecolor=ScientificStyle.WATER_COLOR, alpha=1, zorder=55
            )
        ...
        if np.any(flood_mask > 0):
            cbar_flood = self.create_standard_colorbar(
                im_flood, ax, "Flood Risk", shrink=0.6
            )
        ...
        if show_nl_forecast:
            legend_patches.append(
                mpatches.Patch(
                    color=ScientificStyle.NL_FORECAST_COLOR,
                    alpha=0.1,
                    linewidth=2,
                    edgecolor="darkred",
                    zorder=60,
        ...
        if np.any(netherlands_mask):
            ...
            flood_percentage = (
                (flooded_pixels / total_netherlands_pixels) * 100
                if total_netherlands_pixels > 0
                else 0
            )
    ...
    def visualize_relevance_layer(
        ...
        is_absolute_layer = (
            layer_name.startswith("absolute_") or "absolute" in layer_name.lower()
        )
        ...
        if is_absolute_layer:
            ...
            if len(valid_data) > 0:
                ...
                logger.info(
                    f"Using absolute scaling for {layer_name}: vmin={vmin:.6f}, vmax={vmax:.6f}"
                )
            else:
                vmin, vmax = 0, 1
                logger.warning(
                    f"No valid data found for {layer_name}, using default scaling"
                )
        else:
    ...
    def visualize_risk_layer(
        ...
        # Create zone classification for background
        zones = self.create_zone_classification(
            risk_data.shape, meta["transform"], land_mask
        )
        ...
        # Create data overlay only for Netherlands land areas
        netherlands_mask = (
            (zones == ScientificStyle.LAND_BASE_VALUE)
            & (~np.isnan(risk_data))
            & (risk_data > 0)
        )
        ...
        # Styling
        ax.set_title(
            f"Risk Assessment: {scenario_title}",
    ...
    def create_risk_summary_visualizations(
        self,
    ...
    def create_zone_classification(
        self,
