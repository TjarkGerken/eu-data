...
class AdvancedDataNormalizer:
    ...
    def normalize_hazard_data(self, 
        ...
        return self._apply_sophisticated_normalization(
            ...
            params=self._get_strategy_params(NormalizationStrategy.HAZARD_SOPHISTICATED),
            layer_name="hazard"
        )
    
    def normalize_exposition_data(self,
                                 data: np.ndarray,
        ...
        return self._apply_sophisticated_normalization(
            ...
            params=self._get_strategy_params(NormalizationStrategy.EXPOSITION_OPTIMIZED),
            layer_name="exposition"
        )
    
    def normalize_economic_data(self,
                               data: np.ndarray,
                               economic_mask: Optional[np.ndarray] = None) -> np.ndarray:
        """
    ...
    def _apply_sophisticated_normalization(self,
                                         data: np.ndarray,
        ...
        data_99th = np.percentile(valid_data_values, params.outlier_threshold_percentile)
        ...
        logger.info(f"  95th percentile: {data_95th:.4f}, 99th percentile: {data_99th:.4f}")
        logger.info(f"  Current significant value coverage (>{params.significant_threshold}): {current_significant_pct:.1f}%")
        ...
        if params.preserve_distribution and data_max <= params.target_max:
            logger.info(f"{layer_name}: Data already in optimal range, minimal normalization applied")
            ...
            if data_max < params.target_max * 0.8 and params.enable_boost_factor:
                ...
                logger.info(f"{layer_name}: Applied gentle boost factor: {boost_factor:.3f}")
            ...
            if data_99th > params.conservative_max:
                ...
                logger.info(f"{layer_name}: Applied conservative normalization with factor: {normalization_factor:.3f}")
            else:
            ...
            if not params.preserve_distribution and params.enable_boost_factor:
                if data_max < params.target_max * 0.95:
                    ...
                    normalized_data = masked_data * boost_factor
                    logger.info(f"{layer_name}: Applied full range boost factor: {boost_factor:.3f}")
        ...
        if params.enable_outlier_saturation:
            ...
            if np.any(outlier_mask):
                ...
                normalized_data[outlier_mask] = reasonable_max + params.gentle_saturation_factor * np.tanh(
                    (normalized_data[outlier_mask] - reasonable_max) / params.tanh_scaling_factor
                )
                outlier_count = np.sum(outlier_mask & valid_mask)
                logger.info(f"{layer_name}: Applied gentle saturation to {outlier_count} extreme outlier pixels")
        ...
        if len(final_valid_values) > 0:
            ...
            high_count = np.sum((final_valid_values > 0.6) & (final_valid_values <= 0.8))
            moderate_count = np.sum((final_valid_values > params.significant_threshold) & (final_valid_values <= 0.6))
            low_count = np.sum((final_valid_values > 0.1) & (final_valid_values <= params.significant_threshold))
            ...
            logger.info(f"  High (0.6-0.8): {high_count/total_count*100:.1f}% ({high_count} pixels)")
            logger.info(f"  Moderate (>{params.significant_threshold:.1f}-0.6): {moderate_count/total_count*100:.1f}% ({moderate_count} pixels)")
            logger.info(f"  Low (0.1-{params.significant_threshold:.1f}): {low_count/total_count*100:.1f}% ({low_count} pixels)")
            logger.info(f"  Minimal (<=0.1): {minimal_count/total_count*100:.1f}% ({minimal_count} pixels)")
            logger.info(f"  **Total significant values (>{params.significant_threshold:.1f}): {final_significant_pct:.1f}%**")
            
            # Provide intelligent guidance (adapted from hazard layer)
            if final_significant_pct > params.expected_significant_coverage_max:
                logger.warning(f"{layer_name}: Significant value coverage ({final_significant_pct:.1f}%) is higher than expected "
                ...
                logger.warning(f"{layer_name}: Significant value coverage ({final_significant_pct:.1f}%) is lower than expected "
                              f"(~{params.expected_significant_coverage_min:.0f}%). Consider increasing sensitivity.")
            else:
                logger.info(f"{layer_name}: Significant value coverage ({final_significant_pct:.1f}%) is within expected range "
            ...
            range_utilization = (final_max - final_min) / (params.target_max - params.target_min) * 100
            ...
            if range_utilization < 90 and not params.preserve_distribution:
                logger.info(f"{layer_name}: Range utilization could be improved for better visualization")
        
        return normalized_data.astype(np.float32)


# Convenience functions for backward compatibility and easy usage
def normalize_layer_data(data: np.ndarray, 
                        layer_type: str,
                        valid_mask: Optional[np.ndarray] = None,
                        strategy: Optional[NormalizationStrategy] = None) -> np.ndarray:
    """
...
def ensure_full_range_utilization(data: np.ndarray, 
                                 valid_mask: Optional[np.ndarray] = None,
                                 target_max: float = 1.0,
                                 target_min: float = 0.0) -> np.ndarray:
    """
    ...
    return normalizer._apply_sophisticated_normalization(data, valid_mask, params, "full_range")
