...
class CacheIntegrator:
    ...
    def print_cache_statistics(self):
        ...
        print("=" * 50)
        print(
            f"Cache Status: {'Enabled' if self.cache_manager.enabled else 'Disabled'}"
        )
        ...
        for cache_type, cache_dir in self.cache_manager.cache_dirs.items():
            if cache_dir.exists():
                ...
                print(
                    f"  {cache_type.title()}: {file_count} files, {dir_size_mb:.1f} MB"
                )
...
def create_cached_layers(
    hazard_layer=None,
    exposition_layer=None,
    relevance_layer=None,
    ...
    if hazard_layer:
        cached_layers["hazard"] = integrator.enable_caching_for_hazard_layer(
            hazard_layer
        )

    if exposition_layer:
        cached_layers["exposition"] = integrator.enable_caching_for_exposition_layer(
            exposition_layer
        )

    if relevance_layer:
        cached_layers["relevance"] = integrator.enable_caching_for_relevance_layer(
            relevance_layer
        )

    if risk_assessment:
        cached_layers["risk"] = integrator.enable_caching_for_risk_assessment(
            risk_assessment
        )
...
def invalidate_cache_for_files(file_paths: List[str], config=None):
    ...
    if total_invalidated > 0:
        logger.info(
            f"Invalidated {total_invalidated} cache entries due to file changes"
        )
