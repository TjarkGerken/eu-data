...
class ZeevartDataLoader:
    ...
    def _process_zeevart_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if latest_year not in combined_flow_data["Perioden"].values:
            ...
            latest_year = sorted(
                [
                    ...
                    if "*" in year and "kwartaal" not in year
                ]
            )[-1]
        ...
        aggregated = (
            latest_data.groupby("Nederlandse zeehavens")
            .agg({"Overgeslagen brutogewicht (1 000 ton)": "sum"})
            .reset_index()
        )
        ...
        logger.info(
            f"Processed Zeevart data: {len(aggregated)} port categories for year {latest_year}"
        )
        logger.info(
            f"Total maritime freight volume: {aggregated['freight_value'].sum():,.0f} tonnes"
        )
...
class PortFreightMapper:
    ...
    def map_freight_to_ports(self, zeevart_data: pd.DataFrame) -> gpd.GeoDataFrame:
        ...
        port_with_categories = port_gdf.merge(
            mapping_df, left_on="PORT_ID", right_on="PORT_ID", how="left"
        )

        # Merge with freight data
        port_with_freight = port_with_categories.merge(
            zeevart_data, left_on="Category", right_on="port_category", how="left"
        )

        # Fill missing freight values with 0 and ensure proper float type
        port_with_freight["freight_value"] = (
            port_with_freight["freight_value"].fillna(0).astype(float)
        )
        ...
        if not port_with_freight.empty:
            ...
            for category in port_with_freight["port_category"].dropna().unique():
                if category in zeevart_data["port_category"].values:
                    ...
                    total_freight = zeevart_data[
                        zeevart_data["port_category"] == category
                    ]["freight_value"].iloc[0]
                    ...
                    if len(category_ports) > 0:
                        ...
                        if total_area > 0:
                            ...
                            port_with_freight.loc[category_mask, "freight_value"] = (
                                freight_allocations.values
                            )
                            ...
                            logger.info(
                                f"  {category}: {total_freight:,.0f} tonnes allocated across {num_ports} ports"
                            )
                            logger.info(
                                f"    Area-based allocation range: {min_allocation:,.0f} - {max_allocation:,.0f} tonnes (avg: {avg_allocation:,.0f})"
                            )
                            logger.info(f"    Total area: {total_area:,.0f} m²")
                        else:
                            logger.warning(
                                f"  {category}: Zero total area, using equal distribution fallback"
                            )
        ...
        if nuts_l3_path.exists():
            try:
                ...
                ports_in_study_area = valid_ports[
                    valid_ports.geometry.intersects(study_area)
                ]

                logger.info(
                    f"Clipped to study area: {len(ports_in_study_area)} ports with freight data "
        ...
        logger.info(
            f"Mapped freight to {len(valid_ports)} ports using area-based allocation"
        )
        if len(valid_ports) > 0:
            total_mapped_freight = valid_ports["freight_value"].sum()
            logger.info(
                f"Total mapped maritime freight: {total_mapped_freight:,.0f} tonnes"
            )

            # Log freight distribution by category with area statistics
            freight_by_category = (
                valid_ports.groupby("port_category")
                .agg({"freight_value": "sum", "port_area": "sum"})
            ...
            for category, row in freight_by_category.iterrows():
                ...
                area = row["port_area"]
                logger.info(
                    f"  {category}: {freight:,.0f} tonnes (total area: {area:,.0f} m²)"
                )
...
class CombinedFreightProcessor:
    ...
    def combine_freight_datasets(
        self, nuts_freight_data: pd.DataFrame, port_freight_gdf: gpd.GeoDataFrame
        ...
        if "nuts_freight" in combined_datasets and "port_freight" in combined_datasets:
            logger.info(
                "Created combined freight dataset with both NUTS and port components"
            )

            combined_total = nuts_total + port_total
            logger.info(f"Combined freight total: {combined_total:,.0f} tonnes")
            logger.info(
                f"NUTS share: {(nuts_total / combined_total) * 100:.1f}%, Port share: {(port_total / combined_total) * 100:.1f}%"
            )

        return combined_datasets

    def normalize_combined_freight_data(
        self, combined_datasets: Dict[str, pd.DataFrame]
    ) -> Dict[str, pd.DataFrame]:
        """
        ...
        for dataset_name, dataset in combined_datasets.items():
            if "freight_value" in dataset.columns:
                valid_mask = (dataset["freight_value"] > 0) & dataset[
                    "freight_value"
                ].notna()
                ...
                if valid_mask.sum() > 0:
                    ...
                    normalized_values = self.normalizer.normalize_economic_data(
                        data=freight_values, economic_mask=valid_mask.values
                    )
                    ...
                    logger.info(
                        f"Normalized {dataset_name}: {valid_mask.sum()} valid freight entries"
                    )
                else:
