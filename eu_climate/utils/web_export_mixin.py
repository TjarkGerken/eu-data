...
class WebExportMixin:
    ...
    def save_raster_with_web_exports(
        self,
        data: np.ndarray,
        meta: dict,
        ...
        try:
            ...
            output_meta.update(
                {"driver": "GTiff", "dtype": "float32", "count": 1, "compress": "lzw"}
            )
    ...
    def save_vector_with_web_exports(
        self,
    ...
    def _optimize_clusters_for_web(self, gdf: gpd.GeoDataFrame) -> gpd.GeoDataFrame:
        ...
        try:
                ...
                try:
                    ...
                    if gdf.crs != "EPSG:4326":
                        ...
                        if preserve_topology:
                            simplified = geom_wgs84.simplify(
                        ...
                        optimized_geom = (
                            gpd.GeoSeries([simplified], crs="EPSG:4326")
                            .to_crs(gdf.crs)
                            .iloc[0]
                        )
                    else:
                    ...
                    if (
                        hasattr(optimized_geom, "exterior")
                        and optimized_geom.exterior is not None
                    ):
                        ...
                        if vertex_count > max_vertices:
                            ...
                            while (
                                vertex_count > max_vertices and current_tolerance < 0.01
                            ):  # Max 0.01 degrees
                                current_tolerance *= 1.5
                                if preserve_topology:
                                    test_geom = geom.simplify(
                                        current_tolerance, preserve_topology=True
                                    )
                                else:
                                    test_geom = geom.simplify(current_tolerance)

                                if (
                                    hasattr(test_geom, "exterior")
                                    and test_geom.exterior is not None
                                ):
                            ...
                            logger.debug(
                                f"Reduced polygon vertices from {len(geom.exterior.coords)} to {vertex_count}"
                            )
                    ...
                    logger.warning(
                        f"Failed to optimize geometry for cluster {idx}: {e}"
                    )
            ...
            logger.info(
                f"Web optimization complete: {len(optimized_gdf)} valid polygons"
            )
    ...
    def get_web_export_paths(
        self, base_output_dir: Path, layer_name: str
    ) -> Dict[str, Path]:
        """
