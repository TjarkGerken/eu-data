...
class CachingLayerWrapper:
    ...
    def _get_method_cache_config(self) -> Dict[str, Dict[str, Any]]:
        ...
        config = {
            ...
            "calculate_exposition": {
                ...
                "config_attrs": base_config_attrs
                + ["exposition_weights", "ghs_built_c_class_weights"],
            },
            "normalize_ghs_built_c": {
                "cache_type": "calculations",
                "config_attrs": ["ghs_built_c_class_weights"],
            },
            "normalize_raster": {"cache_type": "calculations", "config_attrs": []},
            ...
            "calculate_integrated_risk": {
                "cache_type": "final_results",
                "config_attrs": base_config_attrs
                + ["risk_weights", "exposition_weights"],
            },
            ...
            "calculate_relevance": {
                ...
                "input_files_attr": [],
                "config_attrs": base_config_attrs
                + ["exposition_weights", "relevance_weights"],
            },
        }
        ...
        for method_name, cache_config in self._method_cache_config.items():
            if hasattr(self._wrapped_layer, method_name):
                ...
                cached_method_func = self._create_cached_method(
                    original_method, method_name, cache_config
                )
    ...
    def _create_cached_method(
        self, original_method: Callable, method_name: str, cache_config: Dict[str, Any]
    ) -> Callable:
        """
        ...
        def cached_wrapper(*args, **kwargs):
            ...
            if not self._cache_manager.enabled:
                logger.info(
                    f"Cache disabled for {self._layer_type}.{method_name}, executing..."
                )
            ...
            logger.info(
                f"Cache miss for {self._layer_type}.{method_name}, executing..."
            )
    ...
    def _generate_method_cache_key(
        self, method_name: str, cache_config: Dict[str, Any], args: tuple, kwargs: dict
    ) -> str:
        """
...
cache_raster_method = lambda **kwargs: CacheAwareMethod(
    cache_type="raster_data", **kwargs
)
cache_calculation_method = lambda **kwargs: CacheAwareMethod(
    cache_type="calculations", **kwargs
)
cache_result_method = lambda **kwargs: CacheAwareMethod(
    cache_type="final_results", **kwargs
)
