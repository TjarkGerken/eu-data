...
class SharedFreightProcessor:
    ...
    def load_and_process_freight_data(
        self,
        ...
        """
        logger.info(
            "Loading freight data using two-stage approach (NUTS L3 + Zeevart ports)"
        )
    ...
    def _load_nuts_road_freight(self) -> pd.DataFrame:
        ...
        if unified_freight_path.exists():
            logger.info(
                f"Loading existing unified freight dataset from {unified_freight_path}"
            )
            nuts_freight_df = pd.read_csv(unified_freight_path)
            return self._process_unified_freight_data(nuts_freight_df)
        else:
            logger.info(
                "Creating unified freight dataset from loading and unloading data"
            )
    ...
    def _create_unified_freight_data(self) -> pd.DataFrame:
        ...
        """
        logger.info(
            "Creating unified freight dataset from loading and unloading sources"
        )
        ...
        if loading_path is None:
            raise FileNotFoundError(
                "Freight loading dataset not found in any expected location"
            )

        # Load freight unloading data
        unloading_paths = [
            self.data_dir
            / "L3-estat_road_go_unloading"
            / "estat_road_go_na_ru3g_en.csv",
            self.data_dir
            / "L3_estat_road_go_unloading"
            / "estat_road_go_na_ru3g_en.csv",
        ...
        if unloading_path is None:
            raise FileNotFoundError(
                "Freight unloading dataset not found in any expected location"
            )
        ...
        unified_data["freight_value_loading"] = unified_data[
            "freight_value_loading"
        ].fillna(0)
        unified_data["freight_value_unloading"] = unified_data[
            "freight_value_unloading"
        ].fillna(0)
        ...
        logger.info(
            f"Created unified freight data: {len(result)} regions with combined loading + unloading values"
        )
        logger.info(
            f"Total road freight volume: {result['freight_value'].sum():,.0f} tonnes"
        )
    ...
    def _process_freight_component_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        logger.info(
            f"Processed freight component data: {len(aggregated)} regions for year {latest_year}"
        )
        if not aggregated.empty:
            logger.info(
                f"Component freight volume: {aggregated['freight_value'].sum():,.0f} tonnes"
            )
    ...
    def _process_unified_freight_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if not processed.empty:
            logger.info(
                f"Total road freight volume: {processed['freight_value'].sum():,.0f} tonnes"
            )

        return processed

    def _load_and_map_zeevart_freight(
        self, nuts_freight_data: pd.DataFrame
        ...
        """
        logger.info(
            "Stage 2: Loading and mapping Zeevart maritime freight data to ports"
        )
        ...
        try:
            ...
            if zeevart_data.empty:
                logger.warning(
                    "No Zeevart data available, using only NUTS road freight"
                )
                return None
            ...
            if port_freight_gdf.empty:
                logger.warning(
                    "No port freight mapping successful, using only NUTS road freight"
                )
                return None
            ...
            combined_datasets = freight_processor.combine_freight_datasets(
                nuts_freight_data, port_freight_gdf
            )

            logger.info(
                "Successfully created enhanced freight datasets with maritime data"
            )
